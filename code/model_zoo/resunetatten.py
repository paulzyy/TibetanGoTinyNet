...
from .modules import ResidualConv, Upsample,Attention_block

@zutils.register_model
class Resunetatten(torch.jit.ScriptModule):
    __constants__ = [
        "c_prime",
        "h_prime",
        "w_prime",
    ...
    def __init__(self, game_params: GameParams, model_params: ModelParams):
        ...
        self.Att1 = Attention_block(F_g=72, F_l=72, F_int=36)
        self.Att2 = Attention_block(F_g=72, F_l=72, F_int=36)
        self.Att3 = Attention_block(F_g=72, F_l=72, F_int=36)
    ...
    def _forward(self, x: torch.Tensor, return_logit: bool):
        ...
        x2 = self.Att2(g=x6, x=x2)
        x7 = torch.cat([x2, x6], dim=1)

        x8 = self.up_residual_conv2(x7)

        x1 = self.Att1(g=x8, x=x1)
