...
class Ghostunet2SECapCA51(torch.jit.ScriptModule):
    __constants__ = [
        "c_prime",
        "h_prime",
        "w_prime",
    ...
    def __init__(self, game_params: GameParams, model_params: ModelParams):
        ...
        self.residual_conv_1 = GhostBottleneck(102,102,102,se_ratio=1)
        # self.residual_conv_1 = GhostModuleVV(72, 72, relu=True, mode='attn')
        self.residual_conv_2 = GhostBottleneck(102, 102, 102, se_ratio=1)
        # self.residual_conv_2 = GhostModuleVV(72, 72, relu=True, mode='attn')
        self.residual_conv_3 = GhostBottleneck(102, 102, 102, se_ratio=1)
        self.bridge = GhostBottleneck(102, 102, 102, se_ratio=1)
        ...
        self.v = nn.Linear(102 * h * w, 1)
        self.pi_logit = nn.Conv2d(
            int(102), c_prime, nnks, stride=stride, padding=padding, dilation=dilation
        )

        self.seblock1 = SqueezeExcite(102)
        self.seblock2 = SqueezeExcite(102)
        self.seblock3 = SqueezeExcite(102)
        ...
        self.encoder_output_atoms = [8, 8, 16, 9, 16, 2]
    ...
    def _forward(self, x: torch.Tensor, return_logit: bool):
        ...
        x5 = self.bridge(x4)
        print(x5.shape)
        conv_3_1_reshaped = x5.view(
            -1, self.encoder_output_dim[3], self.encoder_output_atoms[3], x5.shape[-2], x5.shape[-1])  # [1, 8, 16, 8, 8]
        print(conv_3_1_reshaped.shape)
        ...
        x5 = conv_cap_4_1.view(shape[0], -1, shape[-2], shape[-1])  # [1, 192, 8, 8]
        print(x5.shape)
